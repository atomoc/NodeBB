...
module.exports = function(Meta) {
		...
		Meta.title.parseFragment(uri, language, function(err, title) {
			if (err) {
				...
				if (title && typeof title === 'object') {
					...
					title = validator.escape(title) || validator.escape(title);
				}
				
				if (Meta.config.showBrowserTitle === 1) title = (title ? title + ' | ' : '') + fallbackTitle;
				else title = title && urlFragment ? title : fallbackTitle;
			}
	...
	Meta.title.parseFragment = function (urlFragment, language, callback) {
		...
		if (translated.indexOf(urlFragment) !== -1) {
			...
			require('../categories').getCategoryFields(cid, ['title', 'name'], onParsed);
